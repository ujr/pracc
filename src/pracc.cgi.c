...
static void authorise(const char *user, const char *path);
static int useringroup(const char *user, const char *group);
...
authorise(const char *user, const char *path)
{
    ...
    */
   canpoke = useringroup(user, PRACCPOKE);
   canpeek = useringroup(user, PRACCPEEK) || canpoke;
...
static int
useringroup(const char *user, const char *group)
{
   struct group *gr;
   char **pp;
